...
- Deploy the chart using your New Relic license key (assumes helm v2.x): `helm install --set licenseKey=$NEW_RELIC_LICENSE_KEY --name fluentd .`
- If your are using Helm v3.x you don't need to use the --name flag `helm install --set licenseKey=$NEW_RELIC_LICENSE_KEY fluentd .`
